...
import jp.nyatla.nyartoolkit.test.MarkerSystem;
...
public class NyARGlRender implements INyARSingleCameraSystemObserver
{
	...
	public final void colorSphere(GL i_gl,double rad, double i_x, double i_y, double i_z)
	{
		GL2 gl=i_gl.getGL2();
		GLUT glut = new GLUT();
		float r = 0.0f;
		...
		float b = 1.0f;
		int old_mode = NyARGLUtil.getGlMatrixMode(i_gl);
		gl.glMaterialfv(FRONT, GL_AMBIENT_AND_DIFFUSE, , 10);
		gl.glMatrixMode(GL2.GL_MODELVIEW);
	    gl.glPushMatrix();
	    gl.glTranslated(i_x, i_y, i_z);
	    gl.glRotated(90, 1.0, 0.0, 0.0);
	    glut.glutSolidSphere(rad, 20, 10);
	    gl.glColor3d(r, g, b);
	    gl.glPopMatrix();
	    gl.glMatrixMode(old_mode);
	}
